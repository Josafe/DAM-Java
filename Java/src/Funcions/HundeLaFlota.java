...
/*------------------------------------------Funcions dels barcos!-------------------------------------------------*/
	...
	public static void acorazado (char tauler[][], int contador,int x, int y) //Funcio del acorazado
	{
		...
		int indexi = (int) (0 + Math.random()*x-1+1);
		int indexj = (int) (3 + Math.random()*(y-5)-0+1); //Randomitzarem dues coordenades de la matriu
	...
	public static void buque (char tauler[][], int contador, int x, int y) //Funcio del buques
	{
		...
		for (i = 0; i < contador; i++)
		{
			...
			indexi = (int)(0 + Math.random()*(x-1+1));
			indexj = (int)(2 + Math.random()*((y-3)-0+1));//Randomitzarem dues coordenades de la matriu
	...
	public static void lancha (char tauler[][], int contador, int x, int y) //funcio de les lanches
	{
		...
		int coordenaday = 0;
			...
			for (i = 0; i < contador; i++) //Bucle per a introduir lanchas
			{
				...
				coordenaday = (int) (1 + Math.random()*(tauler[0].length-1)-1+1);

				System.out.println("Index lanches: " + coordenadax + "|" + coordenaday);
				
				if(tauler[coordenadax][coordenaday] == '-') 
				{
	...
	public static void main(String[]args) {
		...
		case 4:
			...
			lancha(tauler,l, x-1, y);
			buque(tauler,b, x-1, y);
			acorazado(tauler,z-1, x, y);
			portaaviones(tauler,p-1, x, y);
